...
import {sendOrderStatusChange} from "./Sendmail"
...
router.put('/product/:id',tokenverify,upload.array('image'),async(req,res,next)=>{
    ...
    try{
     ...
     const images = (req.files as ).map((file) => file.filename);
...
router.put('/update/:id', async (req,res,next)=>{
    ...
    try {
        ...
        if (!order) {
            ...
            console.log(update)
            if (update) {
                const user =  await Usemodel.findById(order.user);

                const products = await ProductSchema.findById(order.product);
               ...
               sendOrderStatusChange(email,product)
...
//add cuponen code 
router.post('/cupon',async(req,res,next)=>{
    try{
...
//view  Users
router.get('/users',tokenverify,async(req,res,next)=>{
    const page  = parseInt(req.query.page as ) || 1;
    const limilt  = 7 
    try{
        const Users = await Usemodel.aggregate([
