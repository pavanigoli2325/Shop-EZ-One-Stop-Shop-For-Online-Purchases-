...
//user register
router.post('/register', upload.single('Avathar'),validate(register), async (req,res,next)=>{
    try {
...
//recomedation 
router.get('/recomedation/:userid',async(req,res,next)=>{
    const userid  = req.params.userid as ;
    try{
...
router.get('/products/:id', verifytoken, async (req,res,next)=>{
    const id  = req.params.id as 
    const page = parseInt(req.query.page as ) || 1
    const limit = 8
    try {
...
//search the product
router.get('/queryproduct/:userId', async (req,res,next)=>{
  const userId = req.params.userId; 
  const page = parseInt(req.query.page as , 10) || 1; // Parse the page parameter from query string
  ...
  try {
    ...
    const { productname, minprice, maxprice, category } = req.query as ;
...
router.get('/product/:userid/:productid', async (req,res,next)=>{
    const userid = req.params.userid as ;
    const productid = req.params.productid as ;
    try {
        const customer = await Usemodel.findById(userid);
        if (!customer) {
            return res.status(404).json({
...
//post ot cart page
router.post('/cartproduct/:userid/:productid',verifytoken,async(req,res,next)=>{
    const userid  = req.params.userid as 
    const productid  = req.params.productid as 
    try{
...
router.get('/cart/:userid',verifytoken,async(req,res,next)=>{
    const userid  = req.params.userid as 
    try{
        const customer = await Usemodel.findById(userid);
        if(!customer){
...
//update the cart 
router.put('/cart/:userid/:id', async (req,res,next)=>{
    const id  = req.params.id as ;
    const userid = req.params.userid as ;
    try {
...
router.delete('/cart/:userid/:productid',verifytoken,async(req,res,next)=>{
    const userid  = req.params.userid as 
    const productid  = req.params.productid as 
    try{
...
router.post('/apply/:userid', async (req,res,next)=>{
    const id  = req.params.userid as ; // corrected variable name
    try {
...
//give feeback
router.post('/feedback/:userid/:productid/:orderid', async (req,res,next)=>{
    const userid = req.params.userid as ;
    const productid = req.params.productid as ;
    const orderid = req.params.orderid as ;
    try {
...
router.get("/cuopncode",async(req,res,next)=>{
    try{
